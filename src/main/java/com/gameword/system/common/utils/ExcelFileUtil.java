...
import com.gameword.system.common.tools.tabletoxls.TableToXls;
...
public class ExcelFileUtil {
    ...
    public static <T> void responseXlsx(HttpServletResponse response, String fileName, String[] headers,
                                        String[] mapHeaders, List<T> list, String pattern) throws Exception {
        response.setCharacterEncoding("utf-8");
        response.setContentType("multipart/form-core");
        response.setHeader("Content-Disposition",
                "attachment;fileName=" + new String(fileName.getBytes("gbk"), "iso-8859-1"));
        InputStream inputStream = getXlsxInputStreamByBean(headers, mapHeaders, list, pattern);
        OutputStream os = response.getOutputStream();
        byte[] b = new byte[1024];
        int length;
        while ((length = inputStream.read(b)) > 0) {
    ...
    public static <T> InputStream getXlsxInputStreamByBean(String[] headers, String[] mapHeaders, List<T> list, String pattern)
            throws IOException {
        File file = createTempFile();
        if (!file.exists()) {
            try {
        ...
        try {
            out = new FileOutputStream(file);
            ExcelUtil.exportExcelX(headers, mapHeaders, list, out, pattern);
            InputStream inputStream = new FileInputStream(file);
            return inputStream;
        } catch (FileNotFoundException e) {
            e.printStackTrace();
    ...
    private static File createTempFile() throws IOException {
        File temp = new File(PropertiesUtil.get("/properties/value.properties", "temp.folder") + System
                .currentTimeMillis());
        return temp;
    }
    ...
    public static <T> void exportXlsx(String path, List<T> list, String[] headers, String[] mapHeaders)
            throws IOException {
        InputStream inputStream = getXlsxInputStreamByBean(headers, mapHeaders, list, null);
        File file = new File(path);
        if (!file.getParentFile().exists()) {
            file.getParentFile().mkdirs();
        }
